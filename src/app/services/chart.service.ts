...
export class ChartService {
	...
	public addData(chart: Chart, dates: moment.Moment[], activities: any[], showChartBy: string, activityParam: string
	): void {
		chart.data.labels = dates.map(date => this.getLabelsFormat(date, showChartBy)),
		chart.data.datasets.forEach(dataset => dataset.data = activities.map( value => this.getFotmatedDataForDataset(activityParam, value) ));
		chart.update();
	}
	 ...
	 */
	public activitiesToChart(dataToChart: DataToChart, showChartBy: string, activityParam: string): any {
		...
		activities.forEach(activity => {
			...
			if (index !== -1) {
				activitiesParamValue[index] += activity[activityParam];
			}
		});
		// console.log(activitiesParamValue[index]);
	...
	public getFotmatedDataForDataset(activityParam: string, value: number): number | any {
